// ═══════════════════════════════════════════════════════════════════════
// Bracelet Cards Admin Module — v11.5 Phase 5c/7
// بوابة الحاج — شركة الأحمدي
// ═══════════════════════════════════════════════════════════════════════
// المحتوى:
//   - initBraceletBusFilter, filterBracelet, renderBraceletTable, viewPilgrimAck
//   - Bulk: toggleAllBracelet, updateBraceletBulkBar, clearBraceletSelection, applyBulkBracelet
//   - Quick: quickBraceletUpdate, exportBraceletReport
//
...
(function injectBraceletStatsStyles(){
  if(document.getElementById('bracelet-stats-styles')) return;
  const style = document.createElement('style');
  style.id = 'bracelet-stats-styles';
  style.textContent = `
  .bracelet-stat-card {
    position: relative;
    overflow: hidden;
  ...
  .bracelet-stat-card.is-active {
    ...
    animation: braceletActivePulse 2s ease-in-out infinite;
  }
  @keyframes braceletActivePulse {
  ...
  .bracelet-stat-card:hover .bracelet-stat-icon {
    transform: scale(1.25) rotate(-5deg);
  }
  .bracelet-stat-number {
    display: inline-block;
  ...
  .bracelet-stat-card:hover .bracelet-stat-badge {
    background: rgba(200,151,26,0.15);
    color: #7a4500;
    transform: scale(1.1);
  }
  .bracelet-stat-card.is-active .bracelet-stat-badge {
    background: #c8971a;
    color: #fff;
    box-shadow: 0 2px 6px rgba(200,151,26,0.4);
  }
  .bracelet-stat-progress-bar {
    position: absolute;
    bottom: 0;
    right: 0;
    height: 3px;
  ...
  .bracelet-stat-progress-bar::after {
    content: '';
    position: absolute;
    top: 0; left: -50%;
    width: 50%; height: 100%;
    background: linear-gradient(90deg, transparent, rgba(255,255,255,0.5), transparent);
    animation: braceletShimmer 2.5s infinite;
  }
  @keyframes braceletShimmer {
    0%   { left: -50%; }
    100% { left: 100%; }
  }
  .bracelet-stat-card:hover .bracelet-stat-progress-bar {
    height: 5px;
  }
  .bracelet-stat-label {
    font-size: 11px;
    ...
    @keyframes braceletNumberFlashUp {
      0%   { color: inherit; transform: scale(1); }
      30%  { color: #1a7a1a; transform: scale(1.25); text-shadow: 0 0 12px rgba(26,122,26,0.4); }
      100% { color: inherit; transform: scale(1); }
    }
    @keyframes braceletNumberFlashDown {
      0%   { color: inherit; transform: scale(1); }
      30%  { color: #c00; transform: scale(0.85); text-shadow: 0 0 12px rgba(192,0,0,0.4); }
      100% { color: inherit; transform: scale(1); }
    }
    .bracelet-stat-number.flash-up   { animation: braceletNumberFlashUp 0.8s ease-out; }
    .bracelet-stat-number.flash-down { animation: braceletNumberFlashDown 0.8s ease-out; }
`;
  document.head.appendChild(style);
})();

// ===== أسوارة القطار =====
window._braceletFilter = '';
const BRACELET_STATUSES = ['لم تُطلب','قيد التجهيز','لدى الإدارة','لدى المشرف','مسلّمة للحاج'];
const BRACELET_COLORS  = ['#888','#1a5fa8','#c8971a','#7a4500','#1a7a1a'];
const BRACELET_BG      = ['#f5f5f5','#e8f0fd','#fff3e0','#fdf0e0','#e8f8e8'];

// v23.0-pre-z: قواعد التسلسل الصارم
function _getAllowedNextStatuses(currentStatus){
  // superadmin يرى كل الخيارات
  if(_isSuperAdmin()) return BRACELET_STATUSES.slice();
  ...
  switch(currentStatus){
    case 'لم تُطلب':
      allowed.push('قيد التجهيز', 'لدى الإدارة');
      break;
    case 'قيد التجهيز':
...
function _isBraceletLocked(pilgrim){
  ...
  return !!(pilgrim['أسوارة_sig'] || pilgrim.bracelet_card_sig);
}
function _isSuperAdmin(){
  return !!(window._currentUser && window._currentUser.role === 'superadmin');
}
function _canReopenBracelet(){
...
function initBraceletBusFilter() {
  const sel = document.getElementById('bracelet-bus-filter');
  if(!sel) return;
  // v22.5.2: حفظ القيمة المختارة قبل إعادة البناء (renderBraceletTable يستدعي هذه الدالة ثم يقرأ .value)
...
function filterBracelet(status) {
  window._braceletFilter = status;
  let activeChip = null;
  ['','لم تُطلب','قيد التجهيز','لدى الإدارة','لدى المشرف','مسلّمة للحاج'].forEach((s,i) => {
    const ids = ['bracelet-f-all','bracelet-f-1','bracelet-f-2','bracelet-f-3','bracelet-f-4','bracelet-f-5'];
...
window._braceletLastUpdate = Date.now();
let _braceletUpdateInterval = null;
...
function _updateBraceletLastUpdateText(){
  const el = document.getElementById('bracelet-last-update-text');
  if(!el) return;
  el.textContent = _formatRelativeTime(window._braceletLastUpdate);
}

function _startBraceletLastUpdateTimer(){
  if(_braceletUpdateInterval) clearInterval(_braceletUpdateInterval);
  _updateBraceletLastUpdateText();
  _braceletUpdateInterval = setInterval(_updateBraceletLastUpdateText, 10000); // كل 10 ثوانٍ
}

function markBraceletUpdated(){
  window._braceletLastUpdate = Date.now();
  _updateBraceletLastUpdateText();
  // flash اللون الأخضر للتأكيد البصري
  const el = document.getElementById('bracelet-last-update');
  if(el){
...
if(document.readyState === 'loading'){
  document.addEventListener('DOMContentLoaded', _startBraceletLastUpdateTimer);
} else {
  _startBraceletLastUpdateTimer();
}

// تصدير للنافذة
window.markBraceletUpdated = markBraceletUpdated;

// v23.0-pre-ll: Skeleton loader أثناء تحميل البيانات
function renderBraceletSkeleton(){
  const tbody = document.getElementById('bracelet-tbody');
  if(!tbody) return;

  const rows = Array.from({length:5}, () => `
    <tr class="bracelet-skeleton-row">
      <td><div class="bracelet-skeleton" style="width:16px;height:16px"></div></td>
      <td><div class="bracelet-skeleton bracelet-skeleton-bar" style="width:20px"></div></td>
      <td><div class="bracelet-skeleton bracelet-skeleton-bar" style="width:160px"></div></td>
      <td><div class="bracelet-skeleton bracelet-skeleton-bar" style="width:100px"></div></td>
      <td><div class="bracelet-skeleton bracelet-skeleton-bar" style="width:100px"></div></td>
      <td><div class="bracelet-skeleton bracelet-skeleton-bar" style="width:60px"></div></td>
      <td><div class="bracelet-skeleton bracelet-skeleton-bar" style="width:100px;height:22px;border-radius:11px"></div></td>
      <td><div class="bracelet-skeleton bracelet-skeleton-bar" style="width:80px"></div></td>
      <td><div class="bracelet-skeleton bracelet-skeleton-bar" style="width:140px;height:30px"></div></td>
      <td><div class="bracelet-skeleton bracelet-skeleton-bar" style="width:90px;height:32px"></div></td>
    </tr>
  `).join('');
  tbody.innerHTML = rows;

  // Mobile cards skeleton
  const mc = document.getElementById('bracelet-mobile-cards');
  if(mc){
    mc.innerHTML = Array.from({length:3}, () => `
      <div style="background:#fff;border:1px solid #e0d5c5;border-radius:12px;padding:14px;box-shadow:0 2px 6px rgba(0,0,0,.06)">
        <div class="bracelet-skeleton" style="width:70%;height:16px;margin-bottom:10px"></div>
        <div style="display:flex;gap:8px;margin-bottom:10px">
          <div class="bracelet-skeleton" style="width:90px;height:14px"></div>
          <div class="bracelet-skeleton" style="width:110px;height:14px"></div>
          <div class="bracelet-skeleton" style="width:60px;height:14px"></div>
        </div>
        <div class="bracelet-skeleton" style="width:120px;height:24px;border-radius:12px"></div>
      </div>
    `).join('');
  }
}
...
// تصديرها للنافذة
window.renderBraceletSkeleton = renderBraceletSkeleton;

function renderBraceletTable(filter) {
  initBraceletBusFilter();
  const search = (document.getElementById('bracelet-search')?.value||'').toLowerCase();
  const busFilter = document.getElementById('bracelet-bus-filter')?.value||'';

  // إحصائيات
  const statsEl = document.getElementById('bracelet-stats');
  if(statsEl) {
    const total = ALL_DATA.length;
    const statuses = [{key:'__total__', label:'الإجمالي', color:'#3d2000', count:total}];
    BRACELET_STATUSES.forEach((s,i) => {
      const count = ALL_DATA.filter(p=>(p['حالة أسوارة القطار']||'لم تُطلب')===s).length;
      statuses.push({key:s, label:s, color:BRACELET_COLORS[i], count:count});
    });
    ...
    statsEl.innerHTML = statuses.map(st => {
      ...
      const onClick = st.key === '__total__' ? "filterBracelet('')" : `filterBracelet('${st.key}')`;
      return `<div class="bracelet-stat-card ${active ? 'is-active' : ''}" onclick="${onClick}" style="background:#fff;border-radius:14px;padding:14px 10px;box-shadow:0 2px 8px rgba(0,0,0,.06);border:2px solid ${borderColor}">
        <span class="bracelet-stat-badge">${pct}%</span>
        <div class="bracelet-stat-icon">${BRACELET_ICONS[st.label]||'📄'}</div>
        <div class="bracelet-stat-number" data-key="${st.key}" style="font-size:28px;font-weight:800;color:${st.color}">${st.count}</div>
        <div class="bracelet-stat-label" style="color:${st.color}">${st.label}</div>
        <div class="bracelet-stat-progress-bar" style="width:${pct}%;background:${st.color}"></div>
      </div>`;
    }).join('');

    // Flash animation فقط للأرقام التي تغيّرت
    requestAnimationFrame(() => {
      statsEl.querySelectorAll('.bracelet-stat-number').forEach(el => {
        const key = el.dataset.key;
        const newVal = parseInt(el.textContent) || 0;
        const oldVal = window._braceletPrevCounts[key];
        ...
        window._braceletPrevCounts[key] = newVal;
      });
    });
  }
  ...
  let list = ALL_DATA.filter(p => {
    const matchFilter = !filter || (p['حالة أسوارة القطار']||'لم تُطلب') === filter;
  ...
  const countEl = document.getElementById('bracelet-count');
  if(countEl) countEl.textContent = `عرض ${list.length} من أصل ${ALL_DATA.length} حاج`;

  const tbody = document.getElementById('bracelet-tbody');
  const empty = document.getElementById('bracelet-empty');
  if(!tbody) return;
  ...
  if(!list.length) {
    ...
    const hasSearchOrBusFilter = (document.getElementById('bracelet-search')?.value||'').trim() || (document.getElementById('bracelet-bus-filter')?.value||'');
    const emptyHTML = hasSearchOrBusFilter
      ? `<tr><td colspan="10" style="padding:40px 20px;text-align:center">
          ...
          <button onclick="document.getElementById('bracelet-search').value='';document.getElementById('bracelet-bus-filter').value='';renderBraceletTable(window._braceletFilter)" style="background:#c8971a;color:#fff;border:none;border-radius:8px;padding:8px 16px;font-size:12px;cursor:pointer;font-family:inherit">🔄 مسح الفلاتر</button>
        </td></tr>`
      : `<tr><td colspan="10" style="padding:40px 20px;text-align:center">
    ...
    if(typeof _renderBraceletMobileCards === 'function'){
      const mc = document.getElementById('bracelet-mobile-cards');
      if(mc){
        const mobileHTML = hasSearchOrBusFilter
          ? `<div style="padding:40px 20px;text-align:center;background:#fff;border-radius:12px;border:1px dashed #e0d5c5">
              ...
              <button onclick="document.getElementById('bracelet-search').value='';document.getElementById('bracelet-bus-filter').value='';renderBraceletTable(window._braceletFilter)" style="background:#c8971a;color:#fff;border:none;border-radius:8px;padding:10px 18px;font-size:13px;cursor:pointer">🔄 مسح الفلاتر</button>
            </div>`
          : `<div style="padding:40px 20px;text-align:center;background:#fff;border-radius:12px;border:1px dashed #e0d5c5">
              <div style="font-size:48px;margin-bottom:10px">${msg.icon}</div>
              <div style="font-size:16px;font-weight:700;color:#3d2000;margin-bottom:6px">${msg.title}</div>
  ...
  tbody.innerHTML = list.map((p,i) => {
    const status = p['حالة أسوارة القطار']||'لم تُطلب';
    const si = BRACELET_STATUSES.indexOf(status);
    const color = BRACELET_COLORS[si]||'#888';
    const bg = BRACELET_BG[si]||'#f5f5f5';
    return `<tr style="border-bottom:1px solid #f5ead0;${i%2===0?'':'background:#fffbf0'}">
      <td style="padding:10px;text-align:center">
        <input type="checkbox" class="bracelet-row-check" data-id="${p['_supabase_id']}" onchange="updateBraceletBulkBar()" style="cursor:pointer;width:15px;height:15px">
      </td>
      ...
      <td style="padding:10px 14px;text-align:center;font-size:11px;color:#aaa">${p['أسوارة_time']||'—'}</td>
      <td style="padding:10px 14px;text-align:center">
        ${(() => {
      ...
      <td style="padding:10px 14px;text-align:center">
        ${(() => {
          const hasPilgrimAck = !!p['أسوارة_sig'];
          const canReopen = _canReopenBracelet() && ((p['حالة أسوارة القطار'] || '').includes('مسلّمة') || (p['حالة أسوارة القطار'] || '').includes('لدى المشرف'));
...
// v23.0-pre-y: رسم أسوارة القطار للجوال
function _renderBraceletMobileCards(list){
  const mc = document.getElementById('bracelet-mobile-cards');
  if(!mc) return;
  ...
  mc.innerHTML = list.map((p,i) => {
    ...
    return `<div class="bracelet-card">
      <input type="checkbox" class="bracelet-row-check bracelet-card-check" data-id="${pid}" onchange="updateBraceletBulkBar()">
      <div class="bracelet-card-header">
        <div>
          <div class="bracelet-card-name">${i+1}. ${name}</div>
        </div>
      </div>
      ...
      <div class="bracelet-card-meta-row">
        ...
        <div style="display:flex;align-items:center;gap:6px">
          ...
          <span class="bracelet-card-date-badge">📅 ${time}</span>
        </div>
      </div>
      <div class="bracelet-card-actions">
        ${(() => {
          const isLocked = status === 'مسلّمة للحاج';
          if(isLocked){
            return `<button onclick="showToast('🔒 الأسوارة مسلّمة للحاج ومقفلة — استخدم 🔓 للرجوع', 'warning')" style="padding:6px 10px;border:1.5px solid #e0d5c5;border-radius:8px;font-size:11px;font-family:inherit;background:#f5ead0;color:#888;cursor:not-allowed;width:100%;text-align:right">🔒 ${status}</button>`;
...
function viewPilgrimBraceletAck(pilgrimId) {
  ...
  <title>إقرار استلام أسوارة القطار</title>
  <style>
  ...
  <div class="header">
    ...
    <div class="header-center">
      ...
      <div style="font-size:15px;font-weight:bold;color:#3d2000;margin-top:4px">تعهد استلام أسوارة القطار</div>
    </div>
  ...
  <div class="sig-section">
    <div class="sig-box">
      ...
      <img class="sig-img" src="${p['أسوارة_sig']||''}" alt="توقيع الحاج">
  ...
  <div class="footer">تم إنشاء هذا الإقرار إلكترونياً — ${p['أسوارة_time']||''}</div>
  <div class="no-print" style="text-align:center;margin-top:20px">
    <button onclick="window.print()" style="padding:10px 30px;background:#3d2000;color:#fff;border:none;border-radius:8px;font-size:14px;cursor:pointer">🔧 تجهيز</button>
  </div>
  </body></html>`);
  w.document.close();
}
...
async function applyBulkBracelet() {
  ...
  let ids = allIds.filter(id => {
    const r = ALL_DATA.find(p=>String(p['_supabase_id'])===String(id));
    const current = r?.['حالة أسوارة القطار'] || 'لم تُطلب';
    const locked = _isBraceletLocked(r) && current !== status;
  ...
  if(status === 'مسلّمة للحاج' && !isSuper){
    ids = ids.filter(id => {
      const r = ALL_DATA.find(p=>String(p['_supabase_id'])===String(id));
      if((r?.['حالة أسوارة القطار'] || r?.bracelet_card_status || '') === 'لدى الإدارة'){ noSupAckSkipped.push(id); return false; }
      return true;
    });
    if(!ids.length){
  ...
  ids.forEach(id => {
    const r = ALL_DATA.find(p=>String(p['_supabase_id'])===String(id));
    beforeMap.set(String(id), { bracelet_card_status: r ? (r['حالة أسوارة القطار'] ?? null) : null });
  });

  try {
    await Promise.all(ids.map(id => window.DB.Pilgrims.update(parseInt(id), { bracelet_card_status: status })));
    ids.forEach(id => {
      const r = ALL_DATA.find(p=>String(p['_supabase_id'])===String(id));
      if(r) r['حالة أسوارة القطار'] = status;
    });
    ...
    if(isSuper && lockedSkipped.length === 0 && allIds.some(id => {
      const r = ALL_DATA.find(p=>String(p['_supabase_id'])===String(id));
      return _isBraceletLocked(r);
    })) bulkMeta.bypass_lock = true;
    ids.forEach(id => {
      const r = ALL_DATA.find(p=>String(p['_supabase_id'])===String(id));
      const changes = _buildFieldChanges(beforeMap.get(String(id)) || {}, { bracelet_card_status: status });
      if(!changes) return;
      _recordAudit({
        action_type:  'update',
        entity_type:  'pilgrim',
    ...
    clearBraceletSelection();
    renderBraceletTable(window._braceletFilter);
  } catch(e) { showToast('خطأ: '+e.message, 'error'); }
}

async function quickBraceletUpdate(pilgrimId, status, selectEl) {
  const pilgrim = ALL_DATA.find(p=>String(p['_supabase_id'])===String(pilgrimId))||{};
  const currentStatus = pilgrim['حالة أسوارة القطار']||'لم تُطلب';
  ...
  try {
    ...
    const changes = _buildFieldChanges({ bracelet_card_status: currentStatus }, { bracelet_card_status: status });
    if(changes){
      const meta = { source: 'admin_bracelet_quick' };
...
async function exportBraceletReport() {
  ...
  const filteredList = ALL_DATA.filter(p => {
    const matchFilter = !currentFilter || (p['حالة أسوارة القطار']||'لم تُطلب') === currentFilter;
    const matchSearch = !search || (p['اسم الحاج']||'').toLowerCase().includes(search) || (p['رقم الهوية']||'').includes(search);
    const matchBus = !busFilter || String(p['رقم الحافلة الخاصة بك']||'') === String(busFilter);
    return matchFilter && matchSearch && matchBus;
  });
  ...
  if(!filteredList.length){
    if(typeof showToast === 'function'){
      showToast('⚠️ لا توجد بيانات للتجهيز بالفلترة الحالية', 'warning');
    } else {
      alert('لا توجد بيانات للتجهيز بالفلترة الحالية');
    }
    return;
  }
  ...
  const rows = filteredList.map((p,i) => {
    const s = p['حالة أسوارة القطار']||'لم تُطلب';
    const si = BRACELET_STATUSES.indexOf(s);
    return `<tr>
      <td>${i+1}</td><td style="text-align:right;font-weight:600">${p['اسم الحاج']||'—'}</td>
      <td>${p['رقم الهوية']||'—'}</td>
      <td>${p['رقم الجوال']||'—'}</td>
      <td>${p['رقم الحافلة الخاصة بك']||'—'}</td>
      <td style="color:${BRACELET_COLORS[si]||'#888'};font-weight:600">${s}</td>
      <td>${p['أسوارة_time']||'—'}</td>
    </tr>`;
  }).join('');
      ...
      <tr><td colspan="7" style="padding-bottom:4mm;border-bottom:3px solid #b8860b;-webkit-print-color-adjust:exact;print-color-adjust:exact">
        <div class="hdr">
          ...
          <div style="text-align:right;width:fit-content;margin:0 auto">
            ...
            <div class="dt-sub">وقت التجهيز: ${timeStr}</div>
...
// v23.1-pre-h: دالة إعادة فتح الأسوارة (placeholder)
function openBraceletReopenModal(pilgrimId){
  if(typeof showToast === 'function'){
    showToast('⚠️ ميزة إعادة فتح الأسوارة قيد التطوير', 'warning');
  }else{
    alert('ميزة إعادة فتح الأسوارة قيد التطوير');
  }
  console.log('[openBraceletReopenModal] called for pilgrim:', pilgrimId);
}

// v23.1-pre-g: دالة تحديث البيانات
async function refreshBraceletData(btnEl){
  const btn = btnEl || document.querySelector('button[onclick*="refreshBraceletData"]');
  if(btn){
    btn.disabled = true;
  ...
  try{
    ...
    // إعادة الرسم
    if(typeof renderBraceletTable === 'function'){
      renderBraceletTable(window._braceletFilter);
    }

    if(typeof markBraceletUpdated === 'function'){
      markBraceletUpdated();
    }
    ...
    console.error('[refreshBraceletData]', e);
    if(typeof showToast === 'function'){
...
// v23.1-pre-g: تصدير الدوال للـ window للاستخدام من HTML
window.renderBraceletTable = renderBraceletTable;
window.filterBracelet = filterBracelet;
window.toggleAllBracelet = toggleAllBracelet;
window.applyBulkBracelet = applyBulkBracelet;
window.clearBraceletSelection = clearBraceletSelection;
window.exportBraceletReport = exportBraceletReport;
window.initBraceletBusFilter = initBraceletBusFilter;
...
window._canReopenBracelet = _canReopenBracelet;
